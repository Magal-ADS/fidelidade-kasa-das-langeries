<?php
// /php/cadastrar_cliente.php (VERSÃO FINAL CORRIGIDA)
...
// Adicionamos a coluna 'data_cadastro' e o valor NOW() para sermos mais explícitos com o banco.
...
// Usamos um nome de query diferente para evitar problemas de cache do PostgreSQL
$stmt = pg_prepare($link, "cadastrar_cliente_query_v2", $sql);

if ($stmt) {
    // A @ na frente do pg_execute suprime o Warning padrão do PHP, pois vamos tratar o erro manualmente logo abaixo
    $result = @pg_execute($link, "cadastrar_cliente_query_v2", array($cpf_limpo, $nome, $whatsapp_limpo, $nascimento_para_banco, $usuario_id));
    ...
    if ($result && pg_num_rows($result) > 0) {
        ...
        // Limpa o CPF que não será mais necessário
        ...
        // Verifica se o erro foi de CPF duplicado
